...
$(function() {
  ...
  function ajaxMailChimpForm($form, $resultElement){
      ...
      $form.submit(function(e) {
          ...
          if (!isValidEmail($form)) {
              var error =  "Lütfen geçerli bir e-mail adresi girin.";
  ...
  function submitSubscribeForm($form, $resultElement) {
      $.post("./sorgu.php", {email:$(".input-email").val()})
        .done(function(data){
            console.log(data);
            if (data.status == 1) {
                var message = "Tebrikler! Başvurunuz kabul edildi! 26 Ocak'ta görüşmek üzere.. :)";
                $resultElement.removeClass('error');
              ...
              if(data.status == 2)
                  var message = "Başvurunuzu maalesef olumlu sonuçlandıramadık. Seneye tekrar denemeye ne dersin?";
              else
                  var message = "Bu e-mail adresiyle yapılmış bir başvuru yok.";
        ...
        .fail(function(error){
              ...
              setTimeout(function() {
                $resultElement.addClass('error');
                $resultElement.html("Sorgulama işlemi teknik bir nedenden dolayı tamamlanamadı. Birazdan tekrar dener misin?");
              }, 150);
